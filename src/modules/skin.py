class MinecraftSkinRenderer:
    def __init__(self, skin, height, slim=False):
        self.skin = skin
    ...
    def _load_skin(self):
        ...
        if (width, height) not in [(64, 64), (64, 32)]:
            new_message(title=language_manager.get("messages.titles.error"), message=language_manager.get("messages.texts.error.skin_load"), icon="cancel", option_1=language_manager.get("messages.answers.ok"))
    ...
    def _composite_layer(self, base, overlay):
        result = base.copy()
        result.paste(overlay, (0, 0), overlay)
        return result

    def _detect_slim(self, skin):
    ...
    def _extract_faces(self, skin, is_slim=False):
        faces = {}
        width, height = skin.size
        ...
        if height == 32:
            ...
            faces['head_front'] = self._composite_layer(
                skin.crop((8, 8, 16, 16)),
                skin.crop((40, 8, 48, 16))
            )
            faces['body_front'] = self._composite_layer(
                skin.crop((20, 20, 28, 32)),
                skin.crop((20, 36, 28, 48))
            )
            if is_slim:
                faces['arm_front'] = self._composite_layer(
                    skin.crop((44, 20, 47, 32)),
                    skin.crop((44, 36, 47, 48))
                )
                faces['arm_front_left'] = self._composite_layer(
                    skin.crop((36, 52, 39, 64)),
                    skin.crop((52, 52, 55, 64))
                )
            else:
                faces['arm_front'] = self._composite_layer(
                    skin.crop((44, 20, 48, 32)),
                    skin.crop((44, 36, 48, 48))
                )
                faces['arm_front_left'] = self._composite_layer(
                    skin.crop((36, 52, 40, 64)),
                    skin.crop((52, 52, 56, 64))
                )
            faces['leg_front'] = self._composite_layer(
                skin.crop((4, 20, 8, 32)),
                skin.crop((4, 36, 8, 48))
            )
            faces['leg_front_left'] = self._composite_layer(
                skin.crop((20, 52, 24, 64)),
                skin.crop((4, 52, 8, 64))
            )
        return faces
    ...
    def create_view(self, skin, is_slim=False):
...
def select_png_file():
    ...
    try:
        ...
        new_message(title=language_manager.get("messages.titles.error"), message=language_manager.get("messages.texts.skin_select") + str(e), icon="cancel", option_1=language_manager.get("messages.answers.ok"))
...
class MinecraftTexturePackCreator:
    def __init__(self, mc_folder, skin_path, pack_name="PLauncher skins"):
    ...
    def create_texture_pack(self):
        """
        Создает текстурпак для версий 1.6+ в виде папки в resourcepacks с pack.mcmeta.
        """
    ...
    def modify_options(self):
        """
        Обновляет options.txt для добавления текстурпака в список.
        ...
        if not os.path.exists(options_path):
            with open(options_path, "w", encoding="utf-8") as f:
                f.write("")  # Записываем пустое содержимое, если файл не существует
    ...
    def _update_option_line(self, lines, key, new_pack):
        """
        Обновляет строку в options.txt, добавляя текстурпак в список.
        """
    ...
    def run(self):
        """
        Запускает полный процесс создания и активации текстурпака.
        """
