...
class CTkButton(CTkBaseClass):
    ...
    def __init__(self,
                 ...
                 compound: str = "left",
                 anchor: str = "center",
                 image_near_text: bool = False,
                 **kwargs):
        
        # transfer basic functionality (bg_color, size, appearance_mode, scaling) to CTkBaseClass
        super().__init__(master=master, bg_color=bg_color, width=width, height=height, **kwargs)
        
        self.image_near_text = image_near_text
    ...
    def _create_grid(self):
        ...
        if self._compound == "right":
            ...
            if self._image_label is not None:
                if self.image_near_text:
                    self._image_label.grid(row=2, column=1, sticky="e")
                else:
                    self._image_label.grid(row=2, column=0, sticky="w", padx=10)
            if self._text_label is not None:
                if self.image_near_text:
                    self._text_label.grid(row=2, column=3, sticky="w")
                else:
                    self._text_label.grid(row=2, column=1, sticky="w")
