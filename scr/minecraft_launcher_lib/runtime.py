...
def get_jvm_runtimes() -> list[str]:
    ...
    manifest_data: RuntimeListJson = requests.get(_JVM_MANIFEST_URL, headers=headers).json()
    jvm_list = []
...
def install_jvm_runtime(
    ...
    if len(manifest_data[platform_string][jvm_version]) == 0:
        return
    platform_manifest: PlatformManifestJson = requests.get(manifest_data[platform_string][jvm_version][0]["manifest"]["url"], headers=headers).json()
    ...
    def install_runtime_file(key: str, value: _PlatformManifestJsonFile) -> None:
        ...
        if value["type"] == "file":
            ...
            if value["executable"]:
                try:
                    subprocess.run(["chmod", "+x", current_path], creationflags=134217728)
                except FileNotFoundError:
                    pass
